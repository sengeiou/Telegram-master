...
import org.telegram.utils.ProxyUtil;
...
public class ProxyListActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public static boolean useProxySettings;
...
//            setWillNotDraw(false);
        ...
        public void setProxy(SharedConfig.ProxyInfo proxyInfo) {
            ...
            String name = ProxyUtil.getProxyName(address);
            if (name != null){
                textView.setText(name);
                Integer imageNum = ProxyUtil.getProxyImage(address);
                switch (imageNum){
                    default:
                        checkImageView.setImageResource(R.drawable.ic_link);
                        break;
                    case 4:
                        checkImageView.setImageResource(R.drawable.ic_disable);
                        break;
                }
            }else {
                textView.setText("Dedicated line");
                checkImageView.setImageResource(R.drawable.ic_link);
//                checkImageView.setOnClickListener(v -> presentFragment(new ProxySettingsActivity(currentInfo)));
            }
            checkImageView.setColorFilter(new PorterDuffColorFilter(Theme.getColor(Theme.key_windowBackgroundWhiteGrayText3), PorterDuff.Mode.MULTIPLY));
        ...
        for (int a = 0, count = SharedConfig.proxyList.size(); a < count; a++) {
            ...
            proxyInfo.proxyCheckPingId = ConnectionsManager.getInstance(currentAccount).checkProxy(proxyInfo.address, proxyInfo.port, proxyInfo.username, proxyInfo.password, proxyInfo.secret, time -> AndroidUtilities.runOnUIThread(() -> {
                ...
                if (time == -1) {
                    ...
                    proxyInfo.ping = 0;
                    ProxyUtil.setImageId(proxyInfo.address, 4);
                } else {
                    proxyInfo.ping = time;
                    proxyInfo.available = true;
                    ProxyUtil.setImageId(proxyInfo.address, 1);
                }
