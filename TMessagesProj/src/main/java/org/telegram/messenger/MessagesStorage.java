...
public class MessagesStorage extends BaseController {
    ...
    public void openDatabase(int openTries) {
        ...
        try {
            ...
            if (createTable) {
                ...
                database.executeFast("CREATE TABLE dialog_filter_teleph(id INTEGER PRIMARY KEY, ord INTEGER, unread_count INTEGER, flags INTEGER, title TEXT, emoticon TEXT)").stepThis().dispose();
                ...
                try {
                    database.executeFast("CREATE TABLE IF NOT EXISTS dialog_filter_teleph(id INTEGER PRIMARY KEY, ord INTEGER, unread_count INTEGER, flags INTEGER, title TEXT, emoticon TEXT)").stepThis().dispose();
                } catch (Exception e) {
                    FileLog.e(e);
                }
    ...
    private void loadDialogFilters() {
        storageQueue.postRunnable(() -> {
            try {
                ...
                SQLiteCursor filtersCursor = database.queryFinalized("SELECT id, ord, unread_count, flags, title, emoticon FROM dialog_filter_teleph WHERE 1");
    ...
    private void saveDialogFilterInternal(MessagesController.DialogFilter filter, boolean atBegin, boolean peers) {
        try {
            ...
            SQLitePreparedStatement state = database.executeFast("REPLACE INTO dialog_filter_teleph VALUES(?, ?, ?, ?, ?, ?)");
    ...
    private void deleteDialogFilterInternal(MessagesController.DialogFilter filter) {
        try {
            ...
            database.executeFast("DELETE FROM dialog_filter_teleph WHERE id = " + filter.id).stepThis().dispose();
            database.executeFast("DELETE FROM dialog_filter_ep WHERE id = " + filter.id).stepThis().dispose();
            database.executeFast("DELETE FROM dialog_filter_pin_v2 WHERE id = " + filter.id).stepThis().dispose();
        } catch (Exception e) {
            FileLog.e(e);
    ...
    public void saveDialogFiltersOrderInternal() {
        try {
            SQLitePreparedStatement state = database.executeFast("UPDATE dialog_filter_teleph SET ord = ?, flags = ? WHERE id = ?");
            for (int a = 0, N = dialogFilters.size(); a < N; a++) {
                MessagesController.DialogFilter filter = dialogFilters.get(a);
                state.requery();
